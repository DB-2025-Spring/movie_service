...
public class LevelController {
    ...
    public ResponseEntity<?> updateLevel(
        ...
        try {
            ...
            return ResponseEntity.ok(new LevelUpdateResponse(
                    levelUpOccurred, 
                    customerInputId, 
                    ticketCount,
                    levelUpOccurred ? "등급이 업그레이드되었습니다! 축하 쿠폰이 발급되었습니다." : "등급 업데이트가 필요하지 않습니다."
            ));
            
        } catch (Exception e) {
            log.error("Failed to update level for user with ticket count: {}", ticketCount, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Failed to update level: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> updateLevelForUser(
        ...
        try {
            ...
            log.error("Failed to update level for user: {} with ticket count: {}", 
                    customerInputId, ticketCount, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Failed to update level: " + e.getMessage());
        }
    }
    ...
    public static class LevelUpdateResponse {
        ...
        public LevelUpdateResponse(boolean levelUpOccurred, String customerInputId, int ticketCount, String message) {
            this.levelUpOccurred = levelUpOccurred;
        ...
        // Getters
        public boolean isLevelUpOccurred() { return levelUpOccurred; }
